...
def configure_url(conf):
    ...
    base_link = add_title(base_link, conf["title"])
...
def work_category(base_link, cat: str):
    if cat in work_fields:
        c = work_fields[cat]
    else:
        c = work_fields["developer"]
...
def add_title(base_link, title):
    if title is not None:
        return base_link + filter_dict["title"].format(title)
    return base_link